...
import { getGitCatalogPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [getGitCatalogPlugin],
    });

    await request(server).get('/api/get-git-catalog/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/get-git-catalog/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/get-git-catalog/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/get-git-catalog/todos/${createdTodoItem.id}`)
